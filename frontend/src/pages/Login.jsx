...
const { mutate} = useMutation({
  ...
  onSuccess: () => {
    ...
    localStorage.setItem("authAdmin", user.f_username);
...
<h2 className='bg-blue-500 py-4 text-white text-center font-semibold text-2xl'>Login Page</h2>
     <div className='flex  justify-center  relative top-32  '>
        ...
        <form className='flex flex-col gap-5 shadow-lg p-10' onSubmit={(e)=>{e.preventDefault();mutate(user)}} >
            ...
            <button type="submit"className='bg-blue-500 py-2 w-1/2 rounded-xl text-white text-center font-semibold text-2xl'>Login</button>
            
            </div>
            <div className="flex justify-center">
            <button type="submit"className='hover:bg-blue-500 border:blue-500 border hover:border-none text-blue-500  py-2 w-1/2 rounded-xl hover:text-white text-center font-semibold text-2xl'><Link to="/register">Register</Link></button>
            </div>
